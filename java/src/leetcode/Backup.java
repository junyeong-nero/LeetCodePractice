...
public class Backup {
	...
	public List<TreeNode> generateTrees(int n) {
		...
		for (TreeNode tree : list) {
			...
			CopyTreeNode(tree, temp.left);
			TreeNode left = new TreeNode();
			CopyTreeNode(temp, left);
			res.add(left);

			temp.right = new TreeNode();
			temp.left = null;
			CopyTreeNode(tree, temp.right);
			TreeNode right = new TreeNode();
			CopyTreeNode(temp, right);
			res.add(right);
		}
		return res;
	}

	public void CopyTreeNode(TreeNode src, TreeNode dst) {
		if (src == null || dst == null)
			return;
		dst.val = src.val;
		if (src.left != null) {
			dst.left = new TreeNode();
			CopyTreeNode(src.left, dst.left);
		}
		if (src.right != null) {
			dst.right = new TreeNode();
			CopyTreeNode(src.right, dst.right);
		}
	}
    ...
    public static long numberOfWeeksM(int[] milestones) {
        int N = milestones.length;
        Arrays.sort(milestones);
        long sum = 0, res = 0;
        for (int n : milestones) sum += n;
    ...
    public int getMinimumDifference3(TreeNode root) {
        ...
        if (root.left != null) {
            ...
            value = Math.min(value, getMinimumDifference3(root.left));
        }
        if (root.right != null) {
            value = Math.min(value, Math.abs(root.val - root.right.val));
            value = Math.min(value, getMinimumDifference3(root.right));
        }
        return value;
    }
